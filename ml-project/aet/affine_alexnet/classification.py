...
def main():
    ...
    # Model
    net = Regressor(6).to(device)
    net = torch.nn.DataParallel(net, device_ids=[0])
    if args.net != '':
        net.load_state_dict(torch.load(args.net))

    model = Classifier(_nChannels=1536, _num_classes=10, _cls_type='MultLayerFC2').to(device)
...
def train(trainloader, net, model, criterion, optimizer, epoch, use_cuda, device):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # compute output
        f1, f2 = net(inputs, inputs, out_feat_keys=['conv3'])
        outputs = model(f1)
