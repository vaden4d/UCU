...
class Classifier(nn.Module):
    def __init__(self, _nChannels, _num_classes, _cls_type):
        ...
        if self.cls_type == 'MultLayer':
                ...
                self.classifier.add_module('BatchNorm_1', nn.BatchNorm1d(nFeats))
                self.classifier.add_module('ReLU_1',      nn.ReLU(inplace=True))
                self.classifier.add_module('Liniear_2',   nn.Linear(nFeats, nFeats // 2, bias=False))
                self.classifier.add_module('BatchNorm_2', nn.BatchNorm1d(nFeats // 2))
                self.classifier.add_module('ReLU_3',      nn.ReLU(inplace=True))
                self.classifier.add_module('Liniear_4',      nn.Linear(nFeats // 2, num_classes))
